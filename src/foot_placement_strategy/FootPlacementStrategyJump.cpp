...
FootPlacementStrategyJump::FootPlacementStrategyJump(LegGroup* legs, TorsoBase* torso, loco::TerrainModelBase* terrain) :
...
bool FootPlacementStrategyJump::loadParameters(const TiXmlHandle& handle) {
  ...
  /* desired */
  TiXmlHandle hFPS(handle.FirstChild("FootPlacementStrategy").FirstChild("Jump"));
  pElem = hFPS.Element();
  if (!pElem) {
...
bool FootPlacementStrategyJump::loadHeightTrajectory(const TiXmlHandle &hTrajectory)
{
  TiXmlElement* pElem;
  int iKnot;
  double t, value;
...
double FootPlacementStrategyJump::getHeightOfTerrainInWorldFrame(const Position& steppingLocationCSw)
{
